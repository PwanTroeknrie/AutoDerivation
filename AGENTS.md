...
一个计算语言学工具，用于为构造语言 (conlang) 自动生成词根、判断词源关系并执行形态屈折变化。
...
- **运行**: `python init.py` (首次需先有 cache.json 并执行索引)

### `etymologist.py` — AI 词源编排器 (核心)
...
| 入口 | 命令 | 用途 |
|------|------|------|
| `etymologist.py` | `python etymologist.py` | 交互式词源决策 REPL |
| `generator.py` | `python generator.py` | 批量词根生成 |
| `init.py` | `python init.py` | 向量库重建 + 单次查询测试 |
| `inflecter.py` | `python inflecter.py` | 屈折测试用例 |
...
- AI 模式通过 `CONFIG["ai_mode"]` 在 `cloud` 和 `local` 之间切换
- DeepSeek API key 硬编码在 `etymologist.py` 中 (敏感信息, 不应提交公开仓库)
